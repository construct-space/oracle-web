// Minimal ISO 3166-1 alpha-2 ↔ numeric lookup. The world-atlas TopoJSON
// dataset keys countries by numeric code (USA = 840), but every other source
// in the codebase — backend payloads, browser locale APIs, flag emojis —
// uses alpha-2. This map bridges the two and is the only place to add new
// codes when MaxMind/DB-IP starts emitting one we don't recognize.
//
// XK (Kosovo) is the canonical user-assigned alpha-2 used by GeoIP vendors;
// world-atlas folds Kosovo into the Serbia path (id 688) so we map it there
// to keep the choropleth painted instead of leaving Kosovo blank.
...
// Some Natural Earth features carry no ISO numeric id because the country
// has no official ISO 3166-1 code (Kosovo is the canonical case). These
// features are distinct geometries in world-atlas — they just need a
// name-keyed bridge to the alpha-2 code our backend uses.
export const NAME_TO_ALPHA2: Record<string, string> = {
  Kosovo: 'XK',
}

export function alpha2ToNumeric(cc: string): string | undefined {
  if (!cc) return undefined
  return ALPHA2_TO_NUMERIC[cc.toUpperCase()]
}

// Reverse table built once. world-atlas keys countries by numeric code so
// we look up alpha-2 by id during render. Kosovo (XK → 688) collides with
// Serbia (RS → 688) in the source data; we pick alpha-2 by insertion order
// and prefer the first key, which is RS (alphabetic earlier). Click on
// that path emits "RS" — Kosovo drill-down still works from the table.
...
export function numericToAlpha2(num: string | number | undefined): string {
  if (num == null) return ''
  return NUMERIC_TO_ALPHA2[String(num).padStart(3, '0')] || ''
}
