...
const osDark = ref(
  typeof window !== 'undefined' ? window.matchMedia?.('(prefers-color-scheme: dark)').matches !== false : true,
)
...
const currentTheme = computed(() => {
  ...
  if (id === 'auto') {
    return osDark.value ? allThemes.value.find((t) => t.id === 'vs-dark')! : allThemes.value.find((t) => t.id === 'vs')!
  }
...
function init() {
  ...
  // Browser-only fork — no Tauri native-theme sync. OS dark-mode is
  // picked up via matchMedia above, which is enough for web. The desktop
  // fork (@construct-space/ui) keeps the Tauri path.
}
