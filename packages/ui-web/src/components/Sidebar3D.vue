...
function sidebarBtnClass(active?: boolean): string {
  const base =
    'flex items-center justify-center size-[42px] rounded-[10px] cursor-pointer transition-all duration-150 border-none'
  if (active)
    return `${base} bg-[color-mix(in_srgb,var(--item-color,var(--app-accent))_15%,transparent)] text-[var(--item-color,var(--app-accent))]`
